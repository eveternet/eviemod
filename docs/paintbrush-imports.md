...
Item PNGs become standard item definitions and models in `resourcepacks/eviemod-paintbrush`, under the reserved `eviemod_imported` namespace. The model identifier includes its preset and normalized PNG SHA-256, so later imports cannot replace another item's texture. Bow inherits `minecraft:item/bow` transforms; sword and generic handheld inherit `minecraft:item/handheld`. One PNG is a static image, including while drawing a bow. No draw frames, animation metadata, remote upload, or custom model-file import is inferred from a flat image.
