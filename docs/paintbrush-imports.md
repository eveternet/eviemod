...
Local checks cover opt-in defaults, config validation, generated definitions, head texture paths, size limits, source-file deletion, and component immutability. Run `./gradlew runClient -PuiSmokeTest -PtextureCapture` for the development fixture. It exercises file-drop import, sprite stitching, resource reloads, a catalog download, worn armor-stand rendering, per-UUID isolation, opt-out, reset and invalid-file recovery. Its screenshots are saved under `run/screenshots/paintbrush-*.png`. These checks are separate from live Hypixel and other-mod/resource-pack compatibility testing.
